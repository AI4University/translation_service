...
for i in range(df.shape[0]):
    ...
    if df['nmt_en'][i] is not None:
        df.loc[i, 'text'] = df['nmt_en'][i]['text']

# Lets explore the number of documents of different languages after the preprocessing
print()
df['lang_after_translation'] = df['text'].apply(detect_language)
...
df = df[['actID', 'Title', 'StartYear', 'EndYear', 'Reference', 'text', 'lang_after_translation']]
df = df[df['lang_after_translation']=='en']

df.to_parquet('data_ingest/match_semanticScholar_researchPortal/match_outputs/projects_translated.parquet', index=False)
